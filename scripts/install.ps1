...
$exe = Join-Path $root 'dist\LightMusic.exe'
...
if (-not $NoShortcut) {
    ...
    $link = Join-Path $desktop '轻音乐.lnk'
    $shortcut = $shell.CreateShortcut($link)
    $shortcut.TargetPath = $exe
    $shortcut.WorkingDirectory = Split-Path -Parent $exe
    $shortcut.IconLocation = "$exe,0"
    $shortcut.Description = '轻音乐 - 轻量级本地音乐播放器'
    $shortcut.Save()
    Write-Host "desktop shortcut: $link" -ForegroundColor Green

    $startMenu = Join-Path ([Environment]::GetFolderPath('Programs')) '轻音乐.lnk'
    try {
        ...
        $shortcut2.Description = '轻音乐 - 轻量级本地音乐播放器'
        $shortcut2.Save()
