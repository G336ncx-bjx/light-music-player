...
- **歌曲列表**：自动扫描音乐文件夹，按「歌名 - 歌手.mp3」解析歌名与歌手（支持一位或多位歌手）。
  - **单击歌名即播放**这一首，顶部有「播放全部 / 随机播放」；
  - 每行右侧的 **＋** 可以把这一首单独加入播放队列，右键还有更多操作；
...
- 右上角工具栏（鼠标移上去才会出现）：**锁定 / 解锁**、字号减、字号加、回到主界面、关闭。
  锁定时整窗鼠标穿透，只有鼠标停在工具栏上时工具栏可以点，所以**锁定状态下依然能用鼠标解锁或关闭**。
- 打开 / 隐藏：主界面顶栏的歌词按钮，快捷键 `D`，或全局快捷键 `Ctrl+Alt+D`。
- 锁定 / 解锁：工具栏上的锁按钮、主界面顶栏的锁形按钮、设置里的开关，或全局快捷键 `Ctrl+Alt+L`。
- 锁定后不会出现在 Alt+Tab 里，也不会抢焦点；字号、颜色、不透明度、翻译开关在「设置 → 桌面歌词」里调整，位置与样式自动记忆。
