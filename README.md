...
- **本地占用**：默认**不缓存**——只临时保存正在听的那一首（Windows 的播放内核不支持网络流，所以做不到完全不写盘），
  切歌 / 退出程序立刻删掉；设置里一个开关可以改成「听过的歌都缓存到本机」（可离线播放、并预取下一首），也可以一键清理。
...
- **完全不落地（默认）**：直接在线播放（HTTP 流，服务端支持 Range，所以拖进度条也能用），
  **硬盘上不留任何音频文件**；设置里一个开关可以改成「听过的歌缓存到本机」（可离线播放），并随时清理。
